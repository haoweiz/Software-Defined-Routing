...
void last_data_packet_response(int sock_index){
    ...
    char *last_data_packet_response = (char*)malloc(CNTRL_RESP_HEADER_SIZE + payload_length);
    char *last_data_packet_header = create_response_header(sock_index, control_code, response_code, payload_length);
    memcpy(last_data_packet_response,last_data_packet_header,CNTRL_RESP_HEADER_SIZE);
    memcpy(last_data_packet_response + CNTRL_RESP_HEADER_SIZE,last_data_packet,payload_length);
    sendALL(sock_index, last_data_packet_response, CNTRL_RESP_HEADER_SIZE + payload_length);
