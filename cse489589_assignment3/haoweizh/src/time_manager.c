...
/* If end greater than begin, return intervals, otherwise return {-1,-1} */
...
/* Return the sum of base and interval */
...
void update_time(int router_socket){
    ...
    LIST_FOREACH(t,&time_list,next){
        if(t->isconnect == TRUE){
            ...
            struct timeval diff_send = getdifftime(current,gettotaltime(t->begin_send_time,send));
    ...
    LIST_FOREACH(t,&time_list,next){
        if(t->isconnect == TRUE){
            if(isinitial == 0){
                ...
                struct timeval diff = getdifftime(result,t->send_time);
                ...
                struct timeval diff_expire = getdifftime(result,t->expire_time);
                if(diff_expire.tv_sec < 0 || diff_expire.tv_usec < 0){
                    result.tv_sec = t->expire_time.tv_sec;
                    result.tv_usec = t->expire_time.tv_usec;
                }
                struct timeval diff_send = getdifftime(result,t->send_time);
